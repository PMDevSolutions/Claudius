...
[`ALLOWED_ORIGIN`](/1.x/configuration/worker/) list. Check the browser console
...
your worker URL in `connect-src`. [Self-hosting](/1.x/deployment/self-hosted/)
...
prompt; use [per-client configs](/1.x/configuration/clients/) with separate
...
React sites should prefer the [component](/1.x/deployment/react/) and share their
existing React.
